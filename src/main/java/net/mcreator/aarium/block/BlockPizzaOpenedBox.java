	...
	public static class BlockCustom extends Block {
		...
		public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
			switch ((EnumFacing) state.getValue(BlockHorizontal.FACING)) {
				...
				case SOUTH :
				default :
					return new AxisAlignedBB(0.75D, 0D, 0.95D, 0.25D, 0.2D, 0D);
				case NORTH :
					return new AxisAlignedBB(0.25D, 0D, 0.05D, 0.75D, 0.2D, 1D);
				case WEST :
					return new AxisAlignedBB(0.05D, 0D, 0.75D, 1D, 0.2D, 0.25D);
				case EAST :
					return new AxisAlignedBB(0.95D, 0D, 0.25D, 0D, 0.2D, 0.75D);
			}
		}
