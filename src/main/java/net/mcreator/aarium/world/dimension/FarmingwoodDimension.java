...
package net.mcreator.aarium.world.dimension;
...
import net.mcreator.aarium.procedures.FarmingwoodPlayerEntersDimensionProcedure;
...
public class FarmingwoodDimension extends AariumModElements.ModElement {
	@ObjectHolder("aarium:farmingwood")
	public static final ModDimension dimension = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public FarmingwoodDimension(AariumModElements instance) {
		super(instance, 26);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("farmingwood"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("aarium:farmingwood")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("aarium:farmingwood"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("aarium:farmingwood"));
	}
		...
		if (event.getTo() == type) {
			{
				...
				FarmingwoodPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(Blocks.OAK_LOG.getDefaultState().getBlock(),
