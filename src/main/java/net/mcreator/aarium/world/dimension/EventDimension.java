...
public class EventDimension extends AariumModElements.ModElement {
	@ObjectHolder("aarium:event")
	public static final ModDimension dimension = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public EventDimension(AariumModElements instance) {
		super(instance, 25);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("event"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("aarium:event")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("aarium:event"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("aarium:event"));
	}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(Blocks.VOID_AIR.getDefaultState().getBlock(),
